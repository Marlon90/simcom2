package de.simcom.games.wargamerd.viewStage;
...
import de.simcom.games.wargamerd.controller.impl.NationControllerImpl;
import de.simcom.games.wargamerd.models.unit.Unit;
import de.simcom.games.wargamerd.physics.Calculator;
...
public class NationView implements ViewStage {
	...
	private HBox getArmorMonitor(Camera camera) {
		...
		subScene.setOnMouseDragged((MouseEvent me) -> {
			calculator.armorMonitorCalculator(map, subScene, me, arD, box, material, sliders.get(0), sliders.get(1),
					sliders.get(2), rotateX, rotateY);
		});
		...
		Label apLabel = new Label("Armor Piercing (AP)");
		Label distanceLabel = new Label("Distance");

		subHolder.getChildren().addAll(distanceLabel, sliders.get(0), armorLabel, sliders.get(2));
			...
			public void handle(ActionEvent event) {
				...
				if (isDigit) {
					...
					if (!(customNumber < 1) && !(customNumber > 2275) && !(customNumber == null)) {
						...
						distanceLog.setText("Value not in range");
					}
				} else {
					distanceLog.setText("Invalid value - Enter a number");
				}
			}
		...
		armorMonitor.getChildren().addAll(apLabel, sliders.get(1), subScene, vMain);
