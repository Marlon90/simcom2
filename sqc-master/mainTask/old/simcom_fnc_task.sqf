...
if !(_group getVariable "task") then {
	...
	_kind = (doctrine getVariable "kind") select 0;
		...
		case "AA" : { 
			...
			switch (_kind) do { 
				case 1 : {  
					...
					[_group,[],["Ambush","Move to the designated position and wait for enemy air units to show up. Then engage.","ambushMark"],
						[0,0,0],1,2,true] call BIS_fnc_taskCreate;
				}; 
				case 2 : {  }; 
